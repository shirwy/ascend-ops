...
def reference_reshape_and_cache(key, key_cache, slot_indices, value=None, value_cache=None):
            ...
            for j in range(chunk_size):
                if idx < flat.numel():
                    ...
                    if value is not None and value_cache is not None:
                        value_cache[block, nh16_idx, block_offset, j] = value[token].flatten()[idx].item()
                    idx += 1
...
if __name__ == "__main__":
    ...
    reference_reshape_and_cache(key, key_cache_cpu, slot_indices, value, value_cache_cpu)
    ...
    ops.reshape_and_cache(key_npu, value_npu, key_cache_npu, value_cache_npu, slot_indices_npu)
    torch.npu.synchronize()
    torch.testing.assert_close(key_cache_cpu, key_cache_npu.cpu(), atol=1e-3, rtol=1e-3)
    torch.testing.assert_close(value_cache_cpu, value_cache_npu.cpu(), atol=1e-3, rtol=1e-3)
    ...
    value_cache_cpu2 = torch.zeros(num_blocks, block_size, nh16, 16, dtype=torch.float16)
    ...
    torch.npu.synchronize()
    torch.testing.assert_close(key_cache_cpu2, key_cache2_npu.cpu(), atol=1e-3, rtol=1e-3)
    torch.testing.assert_close(value_cache2_npu.cpu(), torch.zeros_like(value_cache2_npu.cpu()), atol=1e-6, rtol=1e-6)
